...
//svn提交处理
func CommitSvn(svncommit, build string, projectDir string, allFiles []*common.FileInfo) {
	if svncommit != ""{
		pj, ok:= common.ProjectMap[projectDir]
		var name string
		if ok {
			name = pj.Name
		}else{
		...
		//输出结果
		if err != nil {
			common.FailProject = append(common.FailProject, name)
